...
def imprimir_unidades():
    ...
    if filtros["comuna"]:
        condiciones.append("u.idComuna = %s")
        params.append(filtros["comuna"])

    if filtros["modalidad"]:
        condiciones.append("u.idModalidad = %s")
        params.append(filtros["modalidad"])

    if filtros["q"]:
        term = f"%{filtros['q'].lower()}%"
        condiciones.append(
            "(LOWER(u.nombreUnidad) LIKE %s OR LOWER(u.idUnidad) LIKE %s OR LOWER(u.contactoUnidad) LIKE %s "
            "OR LOWER(u.direccionUnidad) LIKE %s OR LOWER(co.nombreComuna) LIKE %s)"
        )
    ...
    query += " GROUP BY u.idUnidad, u.nombreUnidad, u.contactoUnidad, u.direccionUnidad, u.idComuna, co.nombreComuna, mo.nombreModalidad, u.idModalidad"
    query += " ORDER BY u.nombreUnidad ASC"

    cur.execute(query, params)
    data = cur.fetchall()

    cur.execute("SELECT idComuna, nombreComuna FROM comuna")
    comunas = {str(c["idComuna"]): c["nombreComuna"] for c in cur.fetchall()}

    cur.execute("SELECT idModalidad, nombreModalidad FROM modalidad")
    modalidades = {str(m["idModalidad"]): m["nombreModalidad"] for m in cur.fetchall()}
    cur.close()

    filtros["comuna_nombre"] = comunas.get(filtros["comuna"], None)
    filtros["modalidad_nombre"] = modalidades.get(filtros["modalidad"], None)

    return render_template(
        'Organizacion/print_unidad.html',
        unidades=data,
        filtros=filtros
    )
