...
@tipo_equipo.route("/delete_tipo_equipo/<id>", methods=["GET", "POST"])
@administrador_requerido
...
@tipo_equipo.route("/tipo_equipo/imprimir")
@loguear_requerido
def imprimir_tipo_equipo():
    filtros = {
    ...
    if filtros["q"]:
        condiciones.append("LOWER(te.nombreTipo_equipo) LIKE %s")
        params.append(f"%{filtros['q'].lower()}%")

    if condiciones:
        query += " AND " + " AND ".join(condiciones)

    query += " GROUP BY te.idTipo_equipo, te.nombreTipo_equipo ORDER BY te.nombreTipo_equipo ASC"
