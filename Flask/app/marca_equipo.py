...
@marca_equipo.route('/marca_equipo/imprimir', methods=['GET'])
@loguear_requerido
def imprimir_marca_equipo():
    if "user" not in session:
        flash("No estás autorizado para ingresar a esta ruta", 'warning')
        return redirect("/ingresar")
    ...
    cur = mysql.connection.cursor()
    query = "SELECT * FROM marca_equipo WHERE 1=1"
    params = []

    if filtros["q"]:
        query += " AND LOWER(nombreMarcaEquipo) LIKE %s"
        params.append(f"%{filtros['q'].lower()}%")

    query += " ORDER BY nombreMarcaEquipo ASC"
    ...
    return render_template(
        'Equipo/print_marca_equipo.html',
        marcas=data,
        filtros=filtros
    )
