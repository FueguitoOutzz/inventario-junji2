...
@estado_equipo.route('/estado_equipo/imprimir')
@loguear_requerido
def imprimir_estado_equipo():
    filtros = {
    ...
    if filtros["q"]:
        condiciones.append("LOWER(ee.nombreEstado_equipo) LIKE %s")
        params.append(f"%{filtros['q'].lower()}%")

    if condiciones:
        query += " AND " + " AND ".join(condiciones)

    query += " GROUP BY ee.idEstado_equipo, ee.nombreEstado_equipo ORDER BY ee.nombreEstado_equipo ASC"
    ...
    return render_template(
        'Equipo/print_estado_equipo.html',
        estados=data,
        filtros=filtros
    )
