...
@proveedor.route('/proveedor/imprimir', methods=['GET'])
@loguear_requerido
def imprimir_proveedor():
    if "user" not in session:
        flash("you are NOT authorized")
        return redirect("/ingresar")
    ...
    if filtros["q"]:
        condiciones.append("LOWER(nombreProveedor) LIKE %s")
        params.append(f"%{filtros['q'].lower()}%")

    if condiciones:
        query += " WHERE " + " AND ".join(condiciones)

    query += " ORDER BY nombreProveedor ASC"
