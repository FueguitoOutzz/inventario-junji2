...
// --- Impresión de incidencias ---
function normalizarFiltroIncidencia(valor) {
    if (!valor) return "";
...
function construirFiltrosImpresionIncidencias() {
    return {
        q: normalizarFiltroIncidencia(document.getElementById('printIncidenciaQuery')?.value || ''),
        estado: normalizarFiltroIncidencia(document.getElementById('printIncidenciaEstado')?.value || ''),
        desde: normalizarFiltroIncidencia(document.getElementById('printIncidenciaDesde')?.value || ''),
        hasta: normalizarFiltroIncidencia(document.getElementById('printIncidenciaHasta')?.value || ''),
    };
}

function imprimirIncidencias() {
    const filtros = construirFiltrosImpresionIncidencias();
    const params = new URLSearchParams();
    Object.entries(filtros).forEach(([k, v]) => { if (v) params.append(k, v); });
    const url = params.toString() ? `/incidencia/imprimir?${params.toString()}` : '/incidencia/imprimir';
...
function actualizarTextoBotonImprimirIncidencias() {
    const filtros = construirFiltrosImpresionIncidencias();
    const tiene = Object.values(filtros).some(Boolean);
    const btn = document.getElementById('printIncidenciaButton');
    if (btn) btn.textContent = tiene ? 'Imprimir con filtro' : 'Imprimir todo';
}

function limpiarFiltrosImpresionIncidencias() {
    ['printIncidenciaQuery','printIncidenciaEstado','printIncidenciaDesde','printIncidenciaHasta'].forEach((id) => {
        const el = document.getElementById(id);
        if (el) el.value = '';
    });
    actualizarTextoBotonImprimirIncidencias();
}

document.addEventListener('DOMContentLoaded', () => {
    const btnPrint = document.getElementById('printIncidenciaButton');
    if (btnPrint) btnPrint.addEventListener('click', imprimirIncidencias);

    const btnClear = document.getElementById('clearIncidenciaPrint');
    if (btnClear) btnClear.addEventListener('click', limpiarFiltrosImpresionIncidencias);

    ['printIncidenciaQuery','printIncidenciaEstado','printIncidenciaDesde','printIncidenciaHasta'].forEach((id) => {
        const el = document.getElementById(id);
        if (el) {
            el.addEventListener('input', actualizarTextoBotonImprimirIncidencias);
            el.addEventListener('change', actualizarTextoBotonImprimirIncidencias);
        }
    });

    const modal = document.getElementById('printIncidenciaModal');
    if (modal) modal.addEventListener('shown.bs.modal', limpiarFiltrosImpresionIncidencias);
