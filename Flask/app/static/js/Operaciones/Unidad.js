...
function normalizarFiltroUnidad(valor) {
  if (!valor) return "";
  const limpio = String(valor).trim();
  if (!limpio) return "";
  if (limpio.toLowerCase().startsWith("elija")) return "";
  return limpio;
}

function construirFiltrosImpresionUnidades() {
  return {
    q: normalizarFiltroUnidad(document.getElementById('printUnidadQuery')?.value || ''),
    comuna: normalizarFiltroUnidad(document.getElementById('printUnidadComuna')?.value || ''),
    modalidad: normalizarFiltroUnidad(document.getElementById('printUnidadModalidad')?.value || ''),
  };
}

function imprimirUnidades() {
  const filtros = construirFiltrosImpresionUnidades();
  const params = new URLSearchParams();
  Object.entries(filtros).forEach(([k, v]) => { if (v) params.append(k, v); });
  const url = params.toString() ? `/Unidad/imprimir?${params.toString()}` : '/Unidad/imprimir';
...
function actualizarTextoBotonImprimirUnidades() {
  const filtros = construirFiltrosImpresionUnidades();
  const tiene = Object.values(filtros).some(Boolean);
  const btn = document.getElementById('printUnidadButton');
  if (btn) btn.textContent = tiene ? 'Imprimir con filtro' : 'Imprimir todo';
}

function limpiarFiltrosImpresionUnidades() {
  ['printUnidadQuery','printUnidadComuna','printUnidadModalidad'].forEach((id) => {
    const el = document.getElementById(id);
    if (el) el.value = '';
  });
  actualizarTextoBotonImprimirUnidades();
}

document.addEventListener('DOMContentLoaded', () => {
  const btnPrint = document.getElementById('printUnidadButton');
  if (btnPrint) btnPrint.addEventListener('click', imprimirUnidades);

  const btnClear = document.getElementById('clearUnidadPrint');
  if (btnClear) btnClear.addEventListener('click', limpiarFiltrosImpresionUnidades);

  ['printUnidadQuery','printUnidadComuna','printUnidadModalidad'].forEach((id) => {
    const el = document.getElementById(id);
    if (el) {
      el.addEventListener('input', actualizarTextoBotonImprimirUnidades);
      el.addEventListener('change', actualizarTextoBotonImprimirUnidades);
    }
  });

  const modal = document.getElementById('printUnidadModal');
  if (modal) modal.addEventListener('shown.bs.modal', limpiarFiltrosImpresionUnidades);
