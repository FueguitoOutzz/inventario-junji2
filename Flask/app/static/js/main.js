...
$(document).ready(function () {
  ...
  $("form.funcionarios, form.asignaciones").on("submit", function (event) {
    const hiddenInput = $(this).find(".rut_completo");
    const rutCompletoActual = hiddenInput.val();
    const rutIngresado = $(this).find(".rut-input").val();
    if (!rutCompletoActual && rutIngresado) {
      prepararRutCompleto(this);
    }
    const rutCompleto = hiddenInput.val();
    if (!rutCompleto) {
