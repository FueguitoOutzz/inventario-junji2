...
$(document).ready(function () {
    ...
    // --- Impresión de modelos de equipo ---
    function normalizarFiltroModelo(valor) {
        if (!valor) return "";
    ...
    function construirFiltrosImpresionModelos() {
        return {
            q: normalizarFiltroModelo(document.getElementById('printModeloQuery')?.value || ''),
            tipo: normalizarFiltroModelo(document.getElementById('printModeloTipo')?.value || ''),
            marca: normalizarFiltroModelo(document.getElementById('printModeloMarca')?.value || ''),
        };
    }

    function imprimirModelos() {
        cerrarModalImpresion('printModeloModal');
        const filtros = construirFiltrosImpresionModelos();
        const params = new URLSearchParams();
        Object.entries(filtros).forEach(([key, value]) => {
            if (value) params.append(key, value);
        });
        const url = params.toString() ? `/modelo_equipo/imprimir?${params.toString()}` : '/modelo_equipo/imprimir';
    ...
    function actualizarTextoBotonImprimirModelos() {
        const filtros = construirFiltrosImpresionModelos();
        const tiene = Object.values(filtros).some(Boolean);
        const btn = document.getElementById('printModeloButton');
        if (btn) {
            btn.textContent = tiene ? 'Imprimir con filtro' : 'Imprimir todo';
        }
    }

    function limpiarFiltrosImpresionModelos() {
        ['printModeloQuery','printModeloTipo','printModeloMarca'].forEach((id) => {
            const el = document.getElementById(id);
            if (el) el.value = "";
        });
        actualizarTextoBotonImprimirModelos();
    }

    const btnPrint = document.getElementById('printModeloButton');
    if (btnPrint) {
        btnPrint.addEventListener('click', imprimirModelos);
    }

    const btnClear = document.getElementById('clearModeloPrint');
    if (btnClear) {
        btnClear.addEventListener('click', limpiarFiltrosImpresionModelos);
    }

    ['printModeloQuery','printModeloTipo','printModeloMarca'].forEach((id) => {
        const el = document.getElementById(id);
        if (el) {
            el.addEventListener('input', actualizarTextoBotonImprimirModelos);
            el.addEventListener('change', actualizarTextoBotonImprimirModelos);
        }
    });

    const printModal = document.getElementById('printModeloModal');
    if (printModal) {
        printModal.addEventListener('shown.bs.modal', limpiarFiltrosImpresionModelos);
    }
