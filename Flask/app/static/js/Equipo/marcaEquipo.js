...
// --- Impresión de marcas ---
function normalizarFiltroMarca(valor) {
    if (!valor) return "";
    const limpio = String(valor).trim();
    return limpio || "";
}

function construirFiltrosImpresionMarcas() {
    return { q: normalizarFiltroMarca(document.getElementById('printMarcaQuery')?.value || '') };
}

function imprimirMarcas() {
    const filtros = construirFiltrosImpresionMarcas();
    const params = new URLSearchParams();
    Object.entries(filtros).forEach(([k, v]) => { if (v) params.append(k, v); });
    const url = params.toString() ? `/marca_equipo/imprimir?${params.toString()}` : '/marca_equipo/imprimir';
...
function actualizarTextoBotonImprimirMarcas() {
    const filtros = construirFiltrosImpresionMarcas();
    const tiene = Object.values(filtros).some(Boolean);
    const btn = document.getElementById('printMarcaButton');
    if (btn) btn.textContent = tiene ? 'Imprimir con filtro' : 'Imprimir todo';
}

function limpiarFiltrosImpresionMarcas() {
    const query = document.getElementById('printMarcaQuery');
    if (query) query.value = '';
    actualizarTextoBotonImprimirMarcas();
}

document.addEventListener('DOMContentLoaded', () => {
    const btnPrint = document.getElementById('printMarcaButton');
    if (btnPrint) btnPrint.addEventListener('click', imprimirMarcas);

    const btnClear = document.getElementById('clearMarcaPrint');
    if (btnClear) btnClear.addEventListener('click', limpiarFiltrosImpresionMarcas);

    const query = document.getElementById('printMarcaQuery');
    if (query) query.addEventListener('input', actualizarTextoBotonImprimirMarcas);

    const modal = document.getElementById('printMarcaModal');
    if (modal) modal.addEventListener('shown.bs.modal', limpiarFiltrosImpresionMarcas);
